...
void ScummPAKFile::readIndex(const Common::Path &containerFile, bool isFT) {
	...
	// Read the entry table and the name table each in ONE sequential pass into
	// memory, then parse from there.  The original code seek()ed back and forth
	// between the two tables once per file; over a streamed/buffered container
	// (e.g. the openfpga ISO mount) that thrashes the read buffer badly --
	// ~166 MB of refills for MI1's 2599-entry pak, dominating load time.  Two
	// bulk reads (~180 KB total) replace it.
	const uint32 namesLength = (dataOffset > fileNamesOffset) ? dataOffset - fileNamesOffset : 0;
	_baseStream->seek(fileEntriesOffset, SEEK_SET);
	Common::SeekableReadStream *entryTable = _baseStream->readStream(fileEntriesLength);
	_baseStream->seek(fileNamesOffset, SEEK_SET);
	Common::SeekableReadStream *nameTable = namesLength ? _baseStream->readStream(namesLength) : nullptr;

	for (uint32 i = 0; entryTable && nameTable && i < fileCount; i++) {
		PAKFile pakFile;

		entryTable->seek(i * recordSize, SEEK_SET);
		pakFile.start = !isFT ? entryTable->readUint32LE() : entryTable->readUint64LE();
		pakFile.start += dataOffset;
		entryTable->skip(4); // skip file name offset
		pakFile.len = entryTable->readUint32LE();

		nameTable->seek(curNameOffset, SEEK_SET);
		Common::String fileName = nameTable->readString();
